...
从自建 Claw 项目（`d:\AI\Claude\claude_openclaw\`）迁移到 Fork OpenClaw 上游。
原因：自建项目 ~80% 代码是标准 AI 框架管道，OpenClaw 已全部内置且更成熟。

### 已完成

1. **Fork OpenClaw** → https://github.com/wanhuahua0108/openclaw
2. **Clone 到本地** → `d:\AI\Claude\openclaw\`（已配置 upstream remote）
...
| `d:\AI\Claude\openclaw\` | Fork 本地副本（开发用） |

### 旧项目

`d:\AI\Claude\claude_openclaw\` — 自建 Claw 项目，Phase 1 完成。已不再开发，保留作参考。

## GitHub

- Fork: https://github.com/wanhuahua0108/openclaw
